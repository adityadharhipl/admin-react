...
function AstrologyServices() {
    ...
    useEffect(() => {
        fetchAstrologyServices();
    }, []);

    const fetchAstrologyServices = async () => {
        try {
            setLoading(true);
            const token = localStorage.getItem("User-admin-token");
            const response = await axios.get(
                `${process.env.REACT_APP_BASEURL}/admin/astrology-services`,
                {
                    headers: {
                        'Authorization': `${token}`,
            ...
            if (response.data.success && response.data.data) {
                ...
                setIcons(
                    ICON_LABELS.map((label, index) => {
                        ...
                        return {
                            id: index + 1,
                            label: label, // Always use label from ICON_LABELS
    ...
    const handleSave = async () => {
        try {
            ...
            if (response.data.success) {
                toast.success('Astrology Services saved successfully!');
                fetchAstrologyServices();
            } else {
                toast.error(response.data.message || 'Failed to save astrology services');
            }
        } catch (error) {
            if (error.response?.status === 401) {
                handleUnauthorized();
            } else {
                console.error('Error saving astrology services:', error);
                toast.error(error.response?.data?.message || 'Error saving astrology services');
            }
        } finally {
            setSaving(false);
        }
