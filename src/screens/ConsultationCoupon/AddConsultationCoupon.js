...
// Category options for flat-off and extra
const categoryOptions = [
  //   { value: "flat_off_rupees", label: "Flat Rs. Off" },
  //   { value: "flat_off_percentage", label: "Percent Off" },
  { value: "flat_extra_rupees", label: "Flat Rs. Extra" },
  { value: "flat_extra_percentage", label: "Percent Extra" }
];

function AddConsultationCoupon({ couponId }) {
  ...
  const fetchCouponData = async (id) => {
    try {
      setIsLoading(true);
      const response = await fetch(`${process.env.REACT_APP_BASEURL}/admin/consultation-coupon/${id}`, {
        method: "GET",
        headers: {
      ...
      setValue("category", categoryOptions.find(option => option.value === data.category));
    } catch (error) {
      ...
      <div className="card-body">
        ...
        <form className="row g-4" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="col-md-6">
            ...
            />
            {errors.validFrom && (
              <span className="text-danger">{errors.validFrom.message}</span>
            )}
          </div>
          ...
          <div className="col-md-6">
            <label className="form-label">
              {selectedCategory?.value === "flat_off_percentage" || selectedCategory?.value === "flat_extra_percentage" ? "Define % Extra  " : "Define Flat Extra Rs. "}
            </label>
            <input
              type="number"
              className="form-control"
              placeholder={selectedCategory?.value === "flat_off_percentage" || selectedCategory?.value === "flat_extra_percentage" ? "Enter Extra discount %" : "Enter  Extra discount amount"}
...
export default AddConsultationCoupon;
